...
void rules(CEL **matrix, int N, int M, int K,const char* argv[]) {  
  ...
  while (change_whole < K) {
    ...
    for (int i = 0; i < N; i++) {
      for (int j = 0; j < M; j++) {
        if (matrix[i][j].state == 'X') {
          if (j - 1 >= 0) matrix[i][j - 1].neighbors++;
          if (j + 1 < M) matrix[i][j + 1].neighbors++;
          if (i - 1 >= 0 && j - 1 >= 0) matrix[i - 1][j - 1].neighbors++;
          if (i - 1 >= 0 && j + 1 < M) matrix[i - 1][j + 1].neighbors++;
          if (i - 1 >= 0) matrix[i - 1][j].neighbors++;
          if (i + 1 < N && j - 1 >= 0) matrix[i + 1][j - 1].neighbors++;
          if (i + 1 < N && j + 1 < M) matrix[i + 1][j + 1].neighbors++;
          if (i + 1 < N) matrix[i + 1][j].neighbors++;
        }
      }
    }
    int changes = 0;
    for (int i = 0; i < N; i++) {
      for (int j = 0; j < M; j++) {
        if (matrix[i][j].state == 'X') {
          if (matrix[i][j].neighbors < 2) { /// subpopulare
            matrix[i][j].state = '+';
            changes++;
          } else if (matrix[i][j].neighbors > 3) { /// suprapopulare
            matrix[i][j].state = '+';
            changes++;
          }
        } else if (matrix[i][j].state == '+') {
          if (matrix[i][j].neighbors == 3) { /// reproducere
            matrix[i][j].state = 'X';
            changes++;
          }
        }
